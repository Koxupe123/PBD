...
| :-------------------- | :---------------- | :-------------------------------------------------------------------- |
| `date`            | `date`           | Klucz główny (PK) Dzień wolny.     |
| `name`        | `varchar(100)`           |  Nazwa święta / weekend.                   |
...
| `is_active`   | `bit`           | Określa, czy produkt nadal jest w ofercie (czy nie został usunięty). Domyślnie 1.|
...
Poniższy kod uzupełnia relacje między tabelami.
...
6. Sprawdź, czy dane zamówienie jest możliwe do zrealizowania na czas. Funkcja zwraca wartość 1 (Prawda) lub 0 (Fałsz).

```sql
CREATE FUNCTION dbo.f_CanAcceptOrder (@order_id INT)
RETURNS bit
AS
BEGIN
    DECLARE @result bit;
    ...
    SELECT 
        @result =
            CASE 
                WHEN MIN(CAST(can_produce AS int)) = 1 
                THEN CAST(1 AS bit)
                ELSE CAST(0 AS bit)
            END
    FROM vw_ProductionPossibilityReport
    WHERE order_id = @order_id;
...
Widok pokazuje, czy ukończenie produkcji na czas jest możliwe, biorąc pod uwagę występowanie dni wolnych w całym okresie przeznaczonym na produkcję.

```sql
CREATE VIEW vw_ProductionPossibilityReport AS
SELECT 
    co.order_id,
    od.detail_id,
    ...
    od.quantity,
    p.per_day,
    CEILING(CAST(od.quantity AS float) / p.per_day) AS needed_days,
    ad.available_days,
    CASE 
        WHEN CEILING(CAST(od.quantity AS float) / p.per_day) <= ad.available_days
        THEN CAST(1 AS bit)
        ELSE CAST(0 AS bit)
    END AS can_produce
FROM OrderDetails od
...
CROSS APPLY (
    ...
    FROM (
        SELECT DATEADD(DAY, v.number, CAST(GETDATE() AS date)) AS day
        FROM master..spt_values v
        WHERE v.type = 'P'
          AND DATEADD(DAY, v.number, CAST(GETDATE() AS date)) <= co.expected_date
    ) AS all_days
    WHERE all_days.day NOT IN (SELECT date FROM Calendar)
) AS ad;
```
---
...
3. Automatyczne przeliczanie wartości zamówienia
...
4. Automatyczne dodanie produktu do magazynu po zakończeniu jego produkcji.
...
5. Automatyczne zlecenie produkcji, gdy brakuje towaru.
...
Procedura dodaje rekord do tabeli Clients. Przyjmuje podstawowe dane adresowe (imię, nazwisko, typ, adres). Unikalny identyfikator (client_id) jest generowany wewnątrz procedury, co upraszcza proces dodawania rekordu.
...
BEGIN
    ...
    SELECT @new_id = ISNULL(MAX(client_id), 0) + 1
    FROM Clients;
...
5. Usuwanie produktu. Procedura usuwa produkt z tabeli Products, poprzez zmianę flagi is_active na 0. Dodatkowo sprawdza, czy dany produkt rzeczywiście znajdował się w tabeli.
...
6. Dodawanie zamówienia. Procedura dodaje zamówienie do OrderDetails.
...
ALTER PROCEDURE CreateOrder  
    ...
    @order_items OrderItemList READONLY  
AS  
BEGIN  
    SET NOCOUNT ON;  
    COMMIT TRANSACTION;  
    ...
    BEGIN  
        THROW 50001, 'Klient o podanym ID nie istnieje.', 1;  
        RETURN;  
    END  

    IF NOT EXISTS (SELECT 1 FROM Shippers WHERE shipper_id = @shipper_id)  
    BEGIN  
        THROW 50002, 'Przewoźnik o podanym ID nie istnieje.', 1;  
        RETURN;  
    END  
    ...
    BEGIN  
        THROW 50003, 'Jeden lub więcej produktów nie istnieje.', 1;  
        RETURN;  
    END  
    ...
    BEGIN  
        THROW 50004, 'Nie można utworzyć zamówienia: jeden lub więcej produktów jest nieaktywnych.', 1;  
        RETURN;  
    END  

    DECLARE @new_order_id INT;  
    SELECT @new_order_id = ISNULL(MAX(order_id), 0) + 1  
    FROM ClientOrders WITH (TABLOCKX, HOLDLOCK);  

    DECLARE @start_detail_id INT;  
    SELECT @start_detail_id = ISNULL(MAX(detail_id), 0)  
    FROM OrderDetails WITH (TABLOCKX, HOLDLOCK);  

    INSERT INTO ClientOrders (  
        order_id, client_id, shipper_id, discount, freight,  
        order_date, status, total_amount  
    )  
    VALUES (  
        @new_order_id, @client_id, @shipper_id, @discount, @freight,  
        GETDATE(), 'new', 0  
    );  

    INSERT INTO OrderDetails (detail_id, order_id, product_id, quantity, unit_price)  
    SELECT  
        @start_detail_id + ROW_NUMBER() OVER (ORDER BY oi.product_id),  
        @new_order_id,  
        oi.product_id,  
        oi.quantity,  
        p.base_price  
    ...
    SET @total_amount = @total_amount * (1 - @discount / 100);  
    SET @total_amount = @total_amount + @freight;  
    ...
    SELECT  
        ...
        'Zamówienie utworzone pomyślnie' AS message;  
END;
```

Aby procedura CreateOrder mogła przyjąć listę produktów, zdefiniowano:
